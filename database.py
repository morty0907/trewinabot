...
#Проверка user'a на наличие в БД
...
### Метода для продуктов. Клиентская сторона
### Вывод всех товаров
def get_all_pr():
...
def make_order(user_id):
    ...
    stock_quantity = [sql.execute('SELECT pr_count FROM products WHERE pr_name=?', (i[0],)).fetchone()[0] for i in product_names]
...
def pr_to_db(pr_name, pr_des, pr_price, pr_count, pr_photo):
    if (pr_name,) in sql.execute('SELECT pr_name FROM products;').fetchall():
        ...
        sql.execute('INSERT INTO products (pr_name, pr_des, pr_price, pr_count, pr_photo) '
                    'VALUES (?, ?, ?, ?, ?thgit );', (pr_name, pr_des, pr_price, pr_count, pr_photo ))
       # Фиксируем изменения
        connection.commit()
